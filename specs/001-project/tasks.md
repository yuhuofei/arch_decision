...
| --- | --- | --- | --- |
| T1 | 初始化仓库结构与依赖（§86） | — | 项目可本地启动 |
| T2 | 创建领域模型 + Migration（§64） | T1 | Migration 可应用 |
| T3 | 实现认证（密码 hash / 登录 / refresh） | T2 | 单元测试 + 集成测试（§51,§52） |
| T4 | 实现核心 API 端点（OpenAPI §33） | T3 | 集成测试通过 |
| T5 | 日志 / 观测接入（§47-§49） | T4 | 结构化日志输出 |
| T6 | E2E 关键流程（§53） | T5 | Playwright 通过 |
