...
2. **引用解析只有一处实现**：`scripts/sdd_refs.py`（`gen_traceability.py` 与 `validate_rules.py` 共用）。
   ...
   漏报 44/42 条「未引用」的根因。
3. 迁移脚本：`scripts/migrate_refs.py`（一次性迁移 + 可复跑校验）。
4. 校验脚本：`scripts/validate_rules.py`（裸引用 / 条号存在性 / 文件引用 / Decision Schema / 复杂度预算）。
5. 索引生成：`scripts/gen_traceability.py`（改完引用后必须重跑，否则 `TRACEABILITY.md` 过期）。
