...
│   ├── TRACEABILITY.md           # 4 份来源 → 落点文件反向索引（脚本生成，勿手改）
│   ├── README.md
...
**同一件事只能有一个权威文件**（"canonical rule" 唯一性）：目录归属看本文件，引用写法看 `.sdd/CONVENTIONS.md`，
决策语义看 `.sdd/decision-trees/decision-protocol.md`，版本策略看 `.sdd/knowledge/versioning.md`。
其余文件遇到这些主题**只能引用，不能另立一份**。
